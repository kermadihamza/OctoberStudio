      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.1 }}>
          ...
          <motion.p variants={fadeIn} className="mt-3 max-w-2xl text-neutral-300">
            Une sélection de travaux récents.
          </motion.p>

          <motion.div variants={stagger} className="mt-8 grid gap-6 sm:grid-cols-1 md:grid-cols-2">
  {projects.map((p, i) => (
    <motion.a key={i} variants={fadeIn} href={p.href || "#"} whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }}
      transition={{ type: "spring", stiffness: 300, damping: 26 }}
      className="group relative aspect-[16/10] overflow-hidden rounded-2xl border border-white/10">
      <img
        src={p.img}
        alt={p.title}
        // contain pour Basement, cover pour le reste
        className={`h-full w-full rounded-xl ${p.fit === "contain" ? "object-contain bg-neutral-900" : "object-cover"} transition-transform duration-500 group-hover:scale-[1.03]`}
